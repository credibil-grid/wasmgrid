...
impl HostOutgoingValue for State {
    async fn new_outgoing_value(&mut self) -> wasmtime::Result<Resource<Blob>> {
        tracing::debug!("HostOutgoingValue::new_outgoing_value");
        Ok(self.table().push(Blob::new())?)
    }

    async fn outgoing_value_write_body(
        &mut self, value: Resource<Blob>,
    ) -> wasmtime::Result<Result<Resource<OutputStream>, ()>> {
        tracing::debug!("HostOutgoingValue::outgoing_value_write_body");
        let blob = self.table().get_mut(&value)?;
        let os = Box::new(Blob::from(blob.data.clone())) as Box<dyn HostOutputStream>;
        Ok(Ok(self.table().push(os)?))
    }

    fn drop(&mut self, value: Resource<Blob>) -> wasmtime::Result<()> {
...
impl HostIncomingValue for State {
    ...
    async fn incoming_value_consume_async(
        ...
        let blob = self.table().get_mut(&value)?;
        let s = Box::new(Blob::from(blob.data.clone())) as Box<dyn HostInputStream>;
        let t = InputStream::Host(s);
        Ok(Ok(self.table().push(t)?))
    }

    async fn size(&mut self, value: Resource<Blob>) -> wasmtime::Result<u64> {
