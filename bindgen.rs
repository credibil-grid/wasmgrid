    ...
    mod consumer {
        ...
        impl consumer::Host for super::Nats {
            ...
            fn subscribe_try_receive<'life0, 'async_trait>(
                &'life0 mut self, _client: Resource<Client>, ch: String, t_milliseconds: u32,
            ) -> ::core::pin::Pin<
                Box<
                    ...
                    > = {
                        ...
                        let _result = tokio::spawn(async move {
                            let stream = subscriber
                                .by_ref()
                                .take_until(sleep(Duration::from_millis(t_milliseconds as u64)));
                ...
                Self: 'async_trait,
            {
                Box::pin(async move {
                    ...
                    let __ret: wasmtime::Result<anyhow::Result<(), Resource<Error>>> = {
                        {
                            ::std::io::_print(format_args!("complete_message: {0:?}\n", msg));
                        };
                        Ok(Ok(()))
                    };
                    #[allow(unreachable_code)]
    ...
    mod producer {
        ...
        impl producer::Host for super::Nats {
            ...
            fn send<'life0, 'async_trait>(
                &'life0 mut self, _client: Resource<Client>, ch: String, msg: Vec<Message>,
            ) -> ::core::pin::Pin<
                Box<
                    dyn ::core::future::Future<
                            Output = wasmtime::Result<anyhow::Result<(), Resource<Error>>>,
                ...
                Self: 'async_trait,
            {
                Box::pin(async move {
                    ...
                    let __ret: wasmtime::Result<anyhow::Result<(), Resource<Error>>> = {
                        ...
                        __self.client.publish(ch, data).await.map_or_else(
                            |e| {
...
impl clap::FromArgMatches for Args {
    ...
    fn from_arg_matches_mut(
        ...
        #![allow(deprecated)]
        let v = Args {
            wasm_file: __clap_arg_matches.remove_one::<String>("wasm_file").ok_or_else(|| {
                clap::Error::raw(
    ...
    fn augment_args<'b>(__clap_app: clap::Command) -> clap::Command {
        {
            let __clap_app = __clap_app.group(clap::ArgGroup::new("Args").multiple(true).args({
                let members: [clap::Id; 1usize] = [clap::Id::from("wasm_file")];
                members
            }));
            let __clap_app = __clap_app.arg({
                ...
                let arg = arg.help("The path to the wasm file to run").long_help(None);
                let arg = arg;
                arg
            });
            __clap_app
...
impl ::core::fmt::Debug for Args {
    ...
    fn fmt(&self, f: &mut ::core::fmt::Formatter) -> ::core::fmt::Result {
        ::core::fmt::Formatter::debug_struct_field1_finish(f, "Args", "wasm_file", &&self.wasm_file)
    }
}
                ...
                for i in 0..100 {
                    client
                        .publish(
                            "a",
                            {
                                let res = ::alloc::fmt::format(format_args!("car number {0}", i));
                                res
                            }
                            .into(),
                        )
                ...
                for i in 0..100 {
                    client
                        .publish(
                            "b",
                            {
                                let res = ::alloc::fmt::format(format_args!("ship number {0}", i));
                                res
                            }
                            .into(),
                        )
                ...
                for i in 0..100 {
                    client
                        .publish(
                            "c",
                            {
                                let res = ::alloc::fmt::format(format_args!("plane number {0}", i));
                                res
                            }
                            .into(),
                        )
