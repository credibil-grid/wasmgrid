...
impl<'a> Request<'a> {
    ...
    pub fn host(&self) -> Result<String> {
        let authority = {
            // Check for the x-forwarded-host header first.
            let forwarded_host = self.inner.headers().get(&"x-forwarded-host".to_string());
            if forwarded_host.is_empty() {
                self.inner.authority().unwrap_or_default()
            } else {
                String::from_utf8(forwarded_host[0].clone())?
            }
        };

        let scheme = {
            let forwarded_proto = self.inner.headers().get(&"x-forwarded-proto".to_string());
            if forwarded_proto.is_empty() {
                ...
                String::from_utf8(forwarded_proto[0].clone())?
            }
        };

        // println!("authority: {scheme}://{authority}");
